...
namespace Bdd {
  ...
  uint32_t Bdd2Dot_rec( std::ofstream & f, BddMan<node> & bdd, node x, std::map<uint64_t, uint32_t> & m, std::vector<std::vector<int> > & levels, int & numid ) {
    if ( x == bdd.Const0() ) {
      return 0;
    }
    if ( x == bdd.Const1() ) {
  ...
  template <typename node>
  uint32_t Bdd2DotCedge_rec( std::ofstream & f, BddMan<node> & bdd, node x, std::map<uint64_t, uint32_t> & m, std::vector<std::vector<int> > & levels, int & numid ) {
    x = bdd.Regular( x );
