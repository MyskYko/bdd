...
void run( Bdd::BddMan<node> & bdd, mockturtle::aig_network & aig, mockturtle::klut_network * klut, bool dvr, std::vector<std::string> & pi_names, std::vector<std::string> & po_names, std::string dotname, bool cedge, int verbose, int rows ) {
  if(dvr) {
    bdd.Dvr();
  }
  ...
  node c = bdd.Const1();
  for(int i = rows; i < bdd.GetNumVar(); i++) {
    c = bdd.And(bdd.IthVar(i), c);
  }
  vNodes[0] = bdd.Univ(vNodes[0], c);
...
int main( int argc, char ** argv ) {
  ...
  for(int i = 1; i < argc; i++) {
	...
	break;
      case 'h':
	std::cout << "usage : genvalidfunc <options> your.aig" << std::endl;
	std::cout << "\t-h       : show this usage" << std::endl;
	std::cout << "\t-n       : the number of inputs we focus on [default = " << rows << "]" << std::endl;
  ...
  if(rows <= 0) {
    std::cerr << "please specify the number of inputs we focus on by option -n" << std::endl;
    return 1;
  }
  ...
  case 0:
    {
      ...
      run( bdd, aig, klut, dvr, pi_names, po_names, dotname, cedge, verbose, rows );
    }
    break;
  case 1:
    {
	...
	std::cerr << "the package doesn't use complemented edges" << std::endl;
      ...
      run( bdd, aig, klut, dvr, pi_names, po_names, dotname, 0, verbose, rows );
    }
    break;
  case 2:
    {
